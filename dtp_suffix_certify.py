...
"""
e.g.
CUDA_VISIBLE_DEVICES=1 python dtp_suffix_certify.py --model=llama3 --defender=absorb
"""
...
tokenizer = protected_model.tokenizer

# Certify
os.makedirs("./results/certify/suffix/", exist_ok=True)
results = []
for example_id, (inputs, target) in enumerate(loader):
    ...
    print("-" * 50)
save_testing_result(
    results, f"./results/certify/suffix/{args.defender}_{args.model}_{purify_noise_level}.json", verbose_result=False
)
